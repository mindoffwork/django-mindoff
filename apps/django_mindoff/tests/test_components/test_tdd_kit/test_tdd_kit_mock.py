...
class TestMockApp(MindoffTestCase):
    ...
    def test_is_return_path_returns_tuple_of_name_and_dir(self):
        """is_return_path=True → returns (app_name, temp_dir) instead of just app_name."""
        from pathlib import Path
...
class TestMockModel(MindoffTestCase):
    ...
    def test_optional_fk_is_nullable(self):
        """FK declared as 'optional' has null=True, blank=True on the field."""
    ...
    def test_required_fk_is_not_nullable(self):
        """FK declared as 'required' (default) has null=False."""
        parent = self.mo_mock_model(model_name="ParentModel")
        child = self.mo_mock_model(
            model_name="ChildModel",
    ...
    def test_fields_addon_preserves_attributes(self):
        """Field attributes are preserved end-to-end."""
        fields = {
    ...
    def test_custom_base_model_is_used(self):
        """base_model= is forwarded; the created model inherits from it."""
    ...
    def test_auto_generated_model_names_use_counter_when_base_taken(self):
        """When 'TestModel' is taken the generator produces 'Test1Model', 'Test2Model'…"""
        app_name = self.mo_mock_app()
    ...
    def test_invalid_fk_option_raises(self):
        """A foreign_keys tuple with an unknown option string must raise."""
        app_name = self.mo_mock_app()
...
class TestMockModelFrms(MindoffTestCase):
    ...
    def test_mock_model_frms_col_modified_removed(self):
        """Combined column exclusion + row modification."""
        model_info = [
            {"name": "ParentModel", "fields": copy.deepcopy(FIELDS), "fk": []},
            {
                "name": "ChildModel",
    ...
    def test_is_fk_as_id_false_returns_model_instances(self):
        """is_fk_as_id=False → FK columns contain model instances, not PKs."""
        model_info = [
            {"name": "ParentModel", "fields": copy.deepcopy(FIELDS), "fk": []},
            {
                "name": "ChildModel",
    ...
    def test_is_enforce_db_column_false_uses_field_names(self):
        """is_enforce_db_column=False → DataFrame columns use Django field names, not db_column."""
        fields = {
    ...
    def test_is_uuid_hex_false_uses_uuid_objects(self):
        """is_uuid_hex=False → UUID PK stored as UUID object, not hex string."""
    ...
    def test_omitting_counts_defaults_to_one_row_per_model(self):
        """counts=[] (default) produces 1 row per model."""
...
class TestUpdateMockModelFrms(MindoffTestCase):
    ...
    def test_pk_columns_are_preserved(self):
        """PK values are never replaced during update."""
    ...
    def test_row_count_unchanged_after_update(self):
        """Update replaces values in-place; row count stays the same."""
        df_dict = self._base_df_dict(
            model_info=[
                {"name": "ParentModel", "fields": copy.deepcopy(FIELDS), "fk": []}
            ],
    ...
    def test_is_uuid_hex_false_in_update(self):
        """is_uuid_hex=False propagates correctly through update baking."""
        import uuid as uuid_mod
    ...
    def test_omitting_counts_in_update_defaults_to_one_per_model(self):
        """counts=[] defaults to one per model when source height is already one."""
        df_dict = self._base_df_dict(
            model_info=[
                {"name": "ParentModel", "fields": copy.deepcopy(FIELDS), "fk": []}
            ],
    ...
    def test_exclude_nonexistent_column_raises(self):
        """Unknown exclude_columns entry raises ValueError."""
        df_dict = self._base_df_dict(
            model_info=[
                {"name": "ParentModel", "fields": copy.deepcopy(FIELDS), "fk": []}
            ],
...
# =================================================================
#  🧩 Pure unit tests — no DB, run fast
# =================================================================


class TestValidateOrGenerateModelName:
    """Unit tests for _validate_or_generate_model_name."""
...
class TestNormalizeFkParams:
    """Unit tests for _normalize_fk_and_validate_mockmodel_params."""
...
class TestApplyExcludeColumns:
    """Unit tests for _apply_exclude_columns."""
...
class TestFieldValueGenerator:
    """Unit tests for _tdd_kit.field_value_generator."""
...
class TestValidateOrGenerateAppName:
    """Unit tests for _validate_or_generate_app_name."""
...
class TestUrlResolverHelpers:
    """Unit tests for URL resolver helper functions."""
...
class TestValidateModel:
    """Unit tests for _validate_model helper."""
