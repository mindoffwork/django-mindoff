...
@pytest.mark.django_db(transaction=True)
class TestAPIMixinQueueAcceptance(MindoffTestCase):
    """
    ACCEPTANCE tests for process_mode='queue'.

    Focus:
    - API returns queue_id
    - Polling + streaming URLs are generated
    - run() is NOT executed directly
    """
    ...
    def test_queue_mode_basic_success(self, mock_enqueue):
        queue_id = str(uuid.uuid4())
        mock_enqueue.return_value = queue_id

        api_name = "test_queue_basic_api"
        api_url_name = self._make_api(api_name)
        ...
        assert response.status_code == 200
        assert response.data["message"]["code"] == "SUCCESS"
        assert response.data["data"]["queue_id"] == queue_id
    ...
    def test_queue_persists_db_record(
        self,
        mock_execute_send,
        mock_init_queue,
    ):
        api_name = "test_queue_db_persistence_api"
        api_url_name = self._make_api(api_name)
        ...
        obj = MOQueue.objects.filter(id=queue_id).first()
        assert obj is not None, "MOQueue record was not created"

        # 🔎 Verify DB values
        assert str(obj.id) == queue_id
        assert obj.status == "queued"
        assert obj.api_url_name == api_url_name
        assert obj.request["method"] == "GET"

        # 🔎 Verify Redis + Dramatiq were triggered
        mock_init_queue.assert_called_once()
        mock_execute_send.assert_called_once()

    @patch("apps.django_mindoff.components._api_kit.queue_process.init_queue")
    @patch("apps.django_mindoff.components._api_kit.queue_process.execute_queue.send")
    def test_queue_idempotency_prevents_duplicates(
        self,
        mock_execute_send,
        mock_init_queue,
    ):
        api_name = "test_queue_idempotent_api"
        api_url_name = self._make_api(api_name)
        ...
        response1 = self.client.get(reverse(api_url_name), HTTP_X_TEST="same")
        queue_id_1 = response1.data["data"]["queue_id"]
        response2 = self.client.get(reverse(api_url_name), HTTP_X_TEST="same")
        queue_id_2 = response2.data["data"]["queue_id"]
        assert queue_id_1 == queue_id_2, "Idempotency failed — UUIDs differ"
        from apps.django_mindoff.models import MOQueue

        count = MOQueue.objects.filter(api_url_name=api_url_name).count()
        assert count == 1, "Duplicate MOQueue rows were created"
        mock_init_queue.assert_called_once()
        mock_execute_send.assert_called_once()

    @patch("apps.django_mindoff.components._api_kit.queue_process.init_queue")
    @patch("apps.django_mindoff.components._api_kit.queue_process.execute_queue.send")
    def test_queue_allows_duplicates_when_enabled(
        self,
        mock_execute_send,
        mock_init_queue,
    ):
        api_name = "test_queue_duplicate_allowed_api"
        api_url_name = self._make_api(api_name)
        _modify_api_attributes(
            self._app,
            api_name,
        ...
        response1 = self.client.get(reverse(api_url_name))
        response2 = self.client.get(reverse(api_url_name))
        queue_id_1 = response1.data["data"]["queue_id"]
        queue_id_2 = response2.data["data"]["queue_id"]
        assert queue_id_1 != queue_id_2, "Duplicate UUIDs returned unexpectedly"
        from apps.django_mindoff.models import MOQueue

        count = MOQueue.objects.filter(api_url_name=api_url_name).count()
        assert count == 2, "Expected two separate MOQueue rows"

    @patch("apps.django_mindoff.views.get_queue_status")
    def test_polling_returns_redis_state(self, mock_get_status):
        from apps.django_mindoff.models import MOQueue
        import uuid

        api_name = "test_polling_api"
        api_url_name = self._make_api(api_name)
        queue_id = uuid.uuid4()
        ...
        url = reverse("mo_queue_status_polling", args=[str(queue_id)])
    ...
    @patch("apps.django_mindoff.views.get_queue_status")
    def test_polling_falls_back_to_db_when_redis_unknown(self, mock_get_status):
        from apps.django_mindoff.models import MOQueue
        import uuid
    ...
    @patch("apps.django_mindoff.views.get_queue_status")
    def test_streaming_returns_sse_response(self, mock_get_status):
        from apps.django_mindoff.models import MOQueue
        import uuid

        api_name = "test_streaming_api"
        api_url_name = self._make_api(api_name)
        ...
        _modify_api_attributes(
            ...
            api_name,
            {
                "queue_status_polling_limit": None,
                "queue_status_streaming_limit": None,
            },
            base_path=self._dir,
        )
        ...
        url = reverse("mo_queue_status_streaming", args=[str(queue_id)])
        response = self.client.get(url)
    ...
    @patch("apps.django_mindoff.views.get_queue_status")
    def test_streaming_emits_terminal_event(self, mock_get_status):
        from apps.django_mindoff.models import MOQueue
        import uuid

        api_name = "test_streaming_emit_api"
        api_url_name = self._make_api(api_name)
    ...
    @patch("apps.django_mindoff.views.time.sleep", return_value=None)
    @patch("apps.django_mindoff.views.get_queue_status")
    def test_streaming_emits_only_changed_states(
        self,
        mock_get_status,
        _mock_sleep,
    ):
        ...
        api_name = "test_streaming_state_changes_api"
        api_url_name = self._make_api(api_name)
        _modify_api_attributes(
            self._app,
            api_name,
        ...
        # Should contain exactly one 'processing' and one 'completed'
        assert content.count("processing") == 1
        assert content.count("completed") == 1

    def test_streaming_denies_non_owner(self):
        from apps.django_mindoff.models import MOQueue
        import uuid

        api_name = "test_streaming_owner_api"
        api_url_name = self._make_api(api_name)
        _modify_api_attributes(
            self._app,
            api_name,
        ...
        user1 = User.objects.create_user(username="owner", password=uuid.uuid4().hex)
        user2 = User.objects.create_user(username="intruder", password=uuid.uuid4().hex)
        queue_id = uuid.uuid4()
        MOQueue.objects.create(
            id=queue_id,
        ...
        # Authenticate as wrong user
        self.client.force_authenticate(user=user2)

        url = reverse("mo_queue_status_streaming", args=[str(queue_id)])
        response = self.client.get(url)

        # Depending on your validation behavior:
        # Either 400 (validation error) or 403
        assert response.status_code == 403
        assert response.data["message"]["code"] == "PERMISSION_DENIED"

    @patch("apps.django_mindoff.views.acquire_sse_slot", return_value=False)
    def test_streaming_enforces_concurrency_limit(self, mock_acquire):

        from apps.django_mindoff.models import MOQueue
        import uuid

        api_name = "test_streaming_limit_api"
        api_url_name = self._make_api(api_name)
        ...
        _modify_api_attributes(
            ...
            {
                "queue_status_polling_limit": None,
                "queue_status_streaming_limit": 1,
            },
            base_path=self._dir,
        )
        user = User.objects.create_user(
            username="limit_user", password=uuid.uuid4().hex
        )
        queue_id = uuid.uuid4()
        MOQueue.objects.create(
            id=queue_id,
        ...
        assert response.data["message"]["code"] == "RATE_LIMITED"
    ...
    def test_worker_completes_queue_task(
        self,
        mock_mark_running,
        mock_mark_completed,
        mock_mark_failed,
        ...
        from apps.django_mindoff.models import MOQueue
        from apps.django_mindoff.components._api_kit.queue_process import execute_queue
        import uuid

        api_name = "test_worker_api"
        api_url_name = self._make_api(api_name)
        _modify_api_run_method(
            self._app,
            api_name,
        ...
        execute_queue(str(queue_id))
        obj.refresh_from_db()
        assert obj.status == "completed"
        assert obj.response is not None
        mock_mark_running.assert_called_once_with(str(queue_id))
        mock_mark_completed.assert_called_once_with(str(queue_id))
        mock_mark_failed.assert_not_called()

    @patch("apps.django_mindoff.components._api_kit.queue_process.mark_failed")
    @patch("apps.django_mindoff.components._api_kit.queue_process.mark_completed")
    @patch("apps.django_mindoff.components._api_kit.queue_process.mark_running")
    def test_worker_marks_task_failed_on_exception(
        self,
        mock_mark_running,
        mock_mark_completed,
        mock_mark_failed,
        ...
        api_name = "test_worker_failure_api"
        api_url_name = self._make_api(api_name)
        ...
        assert "ValueError" in obj.error["traceback"]

        mock_mark_running.assert_called_once_with(str(queue_id))
        mock_mark_failed.assert_called_once_with(str(queue_id), error="boom")
        mock_mark_completed.assert_not_called()

    @patch("apps.django_mindoff.components._api_kit.queue_process.execute_queue.send")
    def test_full_queue_pipeline_integration(self, _mock_send):
        ...
        fake_redis.flushdb()
        api_name = "test_full_pipeline_api"
        api_url_name = self._make_api(api_name)
        _modify_api_run_method(
            self._app,
            api_name,
            "        return {'message': 'integration_ok'}",
            base_path=self._dir,
        )
        _modify_api_attribute(
            self._app,
        ...
        with patch.object(redis_state, "redis_client", fake_redis):
            ...
            assert obj.status == "completed"
            assert obj.response is not None
            poll_url = reverse("mo_queue_status_polling", args=[queue_id])
