...
class TestInitManager:
    """Unit tests for init manager — pipeline is fully stubbed; no I/O or subprocesses."""

    # ------------------------------------------------------------------
    # register_subcommand / handler wiring
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # run() — full pipeline order
    #
    # The entire I/O-heavy pipeline (venv creation, pip, django-admin, git)
    # is replaced with lightweight stubs. This is the standard approach for
    # testing orchestration in production codebases: verify the pipeline
    # *ordering and completeness* without executing any real side-effects.
    # ------------------------------------------------------------------
    ...
    def test_project_creator_run_pipeline_is_complete(self, monkeypatch):
        """Regression guard: no pipeline step may be silently dropped."""
        creator = init_manager.DjangoProjectCreator()
        monkeypatch.setattr("builtins.input", lambda _="": "y")
        monkeypatch.setattr(init_manager.os, "chdir", lambda *_: None)
        monkeypatch.setattr(creator, "_prompt_optional_dependencies", lambda: [])
    ...
    # ------------------------------------------------------------------
    # _update_settings — integration-level (file-system, no subprocess)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # _update_urls — integration-level
    # ------------------------------------------------------------------
    ...
    def test_update_urls_adds_include_and_templateview(self, tmp_path):
        ...
        assert '"""URL config."""' not in content  # docstring stripped
    ...
    def test_install_packages_installs_base_and_mindoff(self, monkeypatch):
        ...
        assert "django" in all_args
        assert "djangorestframework" in all_args
    ...
    def test_install_packages_skips_optional_subprocess_when_empty(self, monkeypatch):
        ...
        creator.optional_packages = []
        creator._install_packages()
        # only 2 subprocess calls: base packages + mindoff (no optional call)
