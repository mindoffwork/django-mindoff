...
def _render_crud_methods_markdown(class_node: ast.ClassDef) -> str:
    methods = [
        node
...
def _build_crud_token_markdown(config: Any) -> str | None:
    config_file = Path(config.get("config_file_path", "mkdocs.yml")).resolve()
    root = config_file.parent
    source_path = root / "apps" / "django_mindoff" / "components" / "crud_kit.py"
    if not source_path.exists():
        return None
    tree = ast.parse(source_path.read_text(encoding="utf-8"))
    handler_node = _get_class_node(tree, "MindoffCRUDHandler")
    if handler_node is None:
        return None
    return _render_crud_methods_markdown(handler_node)


def on_page_markdown(markdown: str, config: Any = None, **kwargs: Any) -> str:
    token = "{{ MO_CRUD_KIT_FUNCTIONS }}"
    if token not in markdown:
        return markdown
    cfg = config or {}
    snippet = _build_crud_token_markdown(cfg)
    if not snippet:
        return markdown
