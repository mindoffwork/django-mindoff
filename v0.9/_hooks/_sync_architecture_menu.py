...
def __parse_link_entry(line: str) -> tuple[str, str] | None:
    ...
    if not path.startswith("architecture/") or not path.endswith(".md"):
        return None

    if path.endswith("/index.md"):
        return None

    rel_path = path.split("architecture/", 1)[1]
    return (title, rel_path)


def _extract_architecture_links(config_file: Path) -> list[tuple[str, str]]:
    ...
    for line in lines:
        ...
        if not in_architecture:
            if re.match(r"^\s{2}-\s+Architecture:\s*$", line):
                in_architecture = True
            continue

        # End of Architecture block (next top-level nav group)
        if re.match(r"^\s{2}-\s+[^#].*:\s*$", line):
            break

        # Parse child entries under Architecture
...
def on_page_markdown(markdown: str, config: Any = None, **kwargs: Any) -> str:
    ...
    links = _extract_architecture_links(config_file)
